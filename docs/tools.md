...
| Earnings date accuracy | **Dates run 1 session early.** Volume on rToken session bars peaks 2 sessions after the calendar date in all 8 checkable cases (NVDA x2, MU, QCOM, AMD, AMAT, MRVL, AVGO), i.e. release is calendar + 1 after close. Prequel aligns each event to the peak-volume session (volume only, never price) and shows both dates |
